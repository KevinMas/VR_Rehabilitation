    ...
    private void initContactBones() {
      ...
      for (int fingerIndex = 0; fingerIndex < NUM_FINGERS; fingerIndex++) {
        for (int jointIndex = 0; jointIndex < BONES_PER_FINGER; jointIndex++) {
		  ...
		  GameObject contactBoneObj;
		  if (handDataMode == HandDataMode.PlayerLeft)
		  	contactBoneObj = new GameObject("Contact LeftFingerbone", typeof(CapsuleCollider), typeof(Rigidbody), typeof(ContactBone));
		  else
			contactBoneObj = new GameObject("Contact RightFingerbone", typeof(CapsuleCollider), typeof(Rigidbody), typeof(ContactBone));
